*This master is bigger than most hobgoblins. He is a formidable enemy
...
<worn about body>`   `[`a`` ``robe`` ``splattered`` ``with`` ``goblin`` ``blood`](Robe_Splattered_With_Goblin_Blood "wikilink")  
<wielded>`           a muddy branch`

`A hobgoblin master is carrying:`  
`    `[`rib`` ``bone`` ``from`` ``a`` ``goblin`](Rib_Bone_From_A_Goblin "wikilink")
...
**[Room(s)](:Category:_Rooms "wikilink"):** The highest ascension
(portalable).  
